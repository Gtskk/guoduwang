...
<div class="container">
	...
	<div class="topic panel panel-default">
		<div class="infos panel-heading">
			...
			<div class="votes">
			  	<a data-method="post" href="javascript:void(0);" data-url="{{ route('topics.upvote', $topic->id) }}" title="{{ lang('Up Vote') }}" id="up-vote" class="vote {{ $currentUser && $topic->votes()->ByWhom(Auth::user()->id)->WithType('upvote')->count() ? 'active' :''; }} btn btn-default btn-xs">
			      	<span class="glyphicon glyphicon-chevron-up"></span> {{ $topic->vote_count }}
			  	</a>
			   &nbsp;
			  	<a data-method="post" href="javascript:void(0);" data-url="{{ route('topics.downvote', $topic->id) }}" title="{{ lang('Down Vote') }}" id="down-vote" class="vote {{ $currentUser && $topic->votes()->ByWhom(Auth::user()->id)->WithType('downvote')->count() ? 'active' :''; }} btn btn-default btn-xs">
