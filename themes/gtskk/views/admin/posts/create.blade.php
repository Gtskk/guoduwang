...
@section('content')
    ...
    <div class="row">
        <div class="span12">
            ...
            {{Former::open_for_files( route('admin.posts.store') )}}

            <div class="block">
                <p class="block-heading">{{ trans('cpanel::posts.create_post') }}</p>
                    ...
                    <div class="tabbable">
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane active" id="credentials">
                                ...
                                {{ Former::xlarge_text('title', trans('cpanel::posts.title'))->required() }}
                                {{ Former::xlarge_text('slug', trans('cpanel::posts.slug'))->required() }}
                                {{ Former::file('thumbnail', trans('cpanel::common.thumbnail'))->accept('image') }}
                                {{ Former::select('type', trans('cpanel::common.taxonomy'))->options(array('post' => trans('cpanel::posts.post'), 'new_act' => '最新活动', 'theme_act' => '主题活动'), $type)}}

                                <legend>{{ trans('cpanel::posts.create_legend2') }}</legend>
                                {{ Former::xlarge_text('pubdate', trans('cpanel::posts.pubdate'))->required()->id('datepick') }}

                                <legend>{{ trans('cpanel::posts.create_legend3') }}</legend>
                                {{ Former::textarea('body', trans('cpanel::posts.body'))->required()->id('post_body')->columns(50)->class('') }}
